...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!accountName || !accountNumber || !branchCode) {
      return json({ error: 'EFT payments not configured yet. Please try again shortly.' }, 503);
    }
    ...
    const { data: order, error: orderErr } = await supabase
      .from('orders')
      .select('id, user_id, status, total, currency, order_description')
      .eq('id', orderId)
      .maybeSingle();
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      const { data: existing } = await admin
        ...
        .limit(1)
        .maybeSingle();
      if (!existing) { paymentReference = candidate; break; }
    }
    ...
    if (psaErr) {
      ...
      return json({ error: 'Order could not be prepared for EFT. Please try again.' }, 500);
    }
    ...
    if (mailErr && !mailErr.message.includes('duplicate')) {
      await admin.from('integration_logs').insert({
        integration: 'eft', action: 'create-order',
        status: 'eft_instructions_enqueue_failed',
        payload: { orderId, error: mailErr.message },
      });
    }
    ...
    await admin.from('integration_logs').insert({
      integration: 'eft', action: 'create-order',
      status: 'awaiting_eft',
      payload: { orderId, paymentReference, amount: storedTotal },
    });

    return json({ ok: true, order_id: orderId, payment_reference: paymentReference, bank });
  } catch (err) {
    console.error('eft-create-order error:', err);
    return json({ error: 'EFT order initialisation failed. Please try again.' }, 500);
  }
});
