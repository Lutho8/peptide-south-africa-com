CREATE OR REPLACE FUNCTION tracker.email_queue_dispatch()
  RETURNS void
  LANGUAGE plpgsql
  SECURITY DEFINER
  SET search_path TO ''
...
BEGIN
  ...
  IF (SELECT retry_after_until FROM tracker.email_send_state WHERE id = 1) > now() THEN
    RETURN;
  END IF;

  PERFORM net.http_post(
    url := 'https://eutszmrsukoqqeilzrbv.supabase.co/functions/v1/process-email-queue',
...
GRANT EXECUTE ON FUNCTION "tracker"."email_queue_dispatch"() TO "postgres", "service_role";

REVOKE ALL ON FUNCTION "tracker"."email_queue_dispatch"() FROM PUBLIC;
