...
describe("BuildYourStackPage", () => {
  ...
  it("includes current research-vendor catalogue products in bundle selection", () => {
    ...
    const first = screen.getByLabelText("Vial 1") as HTMLSelectElement;
    expect(first.value).toBe("rt3-reta");
    const optionValues = Array.from(first.options, (option) => option.value);
    expect(optionValues).toContain("rt3-reta");
    expect(optionValues).toContain("tz2-tirz");
  });
  ...
  it("quick-select curated stack fills all 5 slots and adds the bundle during the pre-order window", () => {
    renderBuilder();
    ...
    // 2026-08-31) — the bundle adds without swapping the slot.
  ...
  it("accepts an in-stock ?prefill= deep-link slug during the pre-order window", () => {
    renderBuilder("/build-your-stack?prefill=tesamorelin");
    const first = screen.getByLabelText("Vial 1") as HTMLSelectElement;
    expect(first.value).toBe("tesamorelin");
  });
});
