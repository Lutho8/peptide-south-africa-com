...
import { Download, Printer, Share2, Smartphone, ShieldCheck } from "lucide-react";
...
export default function CoaLabelStudio({ records }: { records: LabelStudioRecord[] }) {
  ...
  const download = async () => {
    ...
    setStatus("Print-ready PNG saved.");
  };
  ...
  const share = async () => {
    ...
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      try {
        await navigator.share({ files: [file], title: `${record.productName} vial label` });
        setStatus("Label shared. Choose Nelko if it appears in your share sheet.");
        return;
      } catch (error) {
    ...
    await download();
    setStatus("Direct sharing is unavailable here. The PNG was downloaded instead.");
  };
      ...
      <div className="grid lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="border-b border-border p-5 sm:p-7 lg:border-b-0 lg:border-r">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Every QR uses a short, PSA-owned verification path. The code remains readable at 14 mm and the destination can be checked before a production run.
          </p>
        ...
        <div className="p-5 sm:p-7">
          ...
          <div className="mt-5 rounded-xl border border-primary/20 bg-primary/5 p-4 text-xs text-muted-foreground">
            <p className="flex items-center gap-2 font-semibold text-foreground"><Smartphone className="h-4 w-4 text-primary" /> Recommended Nelko workflow</p>
            <ol className="mt-2 list-decimal space-y-1 pl-4">
              <li>Open this page on your phone and tap <strong>Share to phone</strong>.</li>
              <li>Choose Nelko from the share sheet if available; otherwise save the PNG and insert it in the Nelko app.</li>
              <li>Select the P21 40 × 14 mm media, keep the artwork at 100%, and use the darkest/highest-quality setting.</li>
              <li>Print one label, apply it to the vial, then scan it with the normal phone camera before printing the batch.</li>
            </ol>
          </div>
