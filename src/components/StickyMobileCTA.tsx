...
export default function StickyMobileCTA() {
  ...
  const hideOn = ["/assessment", "/quiz", "/checkout", "/cart", "/auth", "/account"];
  const allowed = !hideOn.some((p) => location.pathname.startsWith(p));
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-card/95 p-3 backdrop-blur-lg md:hidden">
      ...
      >
        <Sparkles className="h-4 w-4" /> Take Assessment <ArrowRight className="h-4 w-4" />
      </Link>
    </div>
  );
}
